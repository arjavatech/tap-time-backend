...
CREATE TABLE `Employee` (
  ...
  PRIMARY KEY (`EmpID`),
  KEY `CID` (`CID`),
  CONSTRAINT `Employee_ibfk_1` FOREIGN KEY (`CID`) REFERENCES `Company` (`CID`)
);

CREATE TABLE `CompanyReportType` (
  `CompanyReporterEmail` char(255) NOT NULL,
  `CID` char(36) NOT NULL,
  ...
  KEY `CID` (`CID`),
  CONSTRAINT `CompanyReportType_ibfk_1` FOREIGN KEY (`CID`) REFERENCES `Company` (`CID`)
);
...
CREATE TABLE `ContactUS` (
  ...
  KEY `CID` (`CID`),
  CONSTRAINT `ContactUS_ibfk_1` FOREIGN KEY (`CID`) REFERENCES `Company` (`CID`)
);
...
CREATE TABLE `Customer` (
  ...
  KEY `CID` (`CID`),
  CONSTRAINT `Customer_ibfk_1` FOREIGN KEY (`CID`) REFERENCES `Company` (`CID`)
);
...
CREATE TABLE `DailyReportTable` (
  ...
  `TimeWorked` varchar(255) DEFAULT NULL,
  ...
  KEY `CID` (`CID`),
  KEY `EmpID` (`EmpID`),
  CONSTRAINT `DailyReportTable_ibfk_1` FOREIGN KEY (`CID`) REFERENCES `Company` (`CID`),
  CONSTRAINT `DailyReportTable_ibfk_2` FOREIGN KEY (`EmpID`) REFERENCES `Employee` (`EmpID`)
);
...
CREATE PROCEDURE `spGetEmployeeDailyReport`(IN p_cid CHAR(36),  IN reportDate DATE)
BEGIN
    SELECT 
        CONCAT(Employee.FName, " ", Employee.LName) AS "Name", 
        Employee.PIN AS "Pin", 
